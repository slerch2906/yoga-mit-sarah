-- ============================================================================
-- Bugfix (Sarah 2026-08-18): Nachrücken ohne echten freien Platz
-- ============================================================================
-- Problem: process_cancellation_full() und process_cancellation_with_waitlist()
-- wurden nach JEDER Stornierung blind aufgerufen und haben versucht jemanden
-- von der Warteliste nachrücken zu lassen -- ohne vorher zu prüfen, ob die
-- Stunde dadurch wirklich unter die erlaubte Platzzahl gefallen ist.
--
-- Konkreter Sarah-Fall: Stunde mit 11/10 Plätzen (Dummy als Reserve-Platz).
-- Dummy gelöscht -> 10/10, also weiterhin exakt voll. Trotzdem wurde der
-- wartende Yogi nachgerückt, weil KEINE der beiden Funktionen die reale
-- Platzzahl prüft, sondern jede Stornierung blind als "ein Platz ist frei"
-- interpretiert.
--
-- Bei Admin-Aktionen griff bisher kein Schutz, weil der einzige bestehende
-- Kapazitäts-Check (enforce_session_max_spots_on_update-Trigger) Admins
-- bewusst ausnimmt. Bei Yogi-Selbstabmeldung "funktionierte" es bisher nur
-- zufällig richtig: derselbe Trigger blockierte die fehlerhafte Nachrück-
-- Buchung mit einer Exception -- die aber vom Frontend still abgefangen
-- wird (kein Fehler sichtbar, aber auch KEINE Benachrichtigung/Nachrücken
-- für den wartenden Yogi, falls tatsächlich mal kein Platz frei war).
--
-- Fix: Beide Funktionen prüfen jetzt selbst, GANZ AM ANFANG, ob aktuell
-- wirklich weniger aktive Buchungen als Plätze vorhanden sind -- exakt
-- dieselbe Konvention wie enforce_session_max_spots_on_update():
--   COALESCE(session.max_spots, course.max_spots), NULL/0 = kein Limit,
--   aktive Buchungen (status='active') gegen dieses Limit gezählt.
-- Ist die Stunde weiterhin voll, wird gar nicht erst versucht nachzurücken
-- (process_cancellation_full -> mode 'noop', process_cancellation_with_waitlist
-- -> promoted=null, keine Nachrück-/Notify-Aktion) -- unabhängig davon, ob
-- Admin oder Yogi die auslösende Aktion war.
-- ============================================================================
...
BEGIN
  ...
  -- Bugfix (Sarah 2026-08-18): erst PRÜFEN ob wirklich ein Platz frei ist,
  -- bevor überhaupt versucht wird nachzurücken/Spätangebote zu verschicken.
  SELECT COALESCE(s.max_spots, c.max_spots) INTO v_max_spots
    FROM sessions s LEFT JOIN courses c ON c.id = s.course_id
   WHERE s.id = p_session_id;

  IF v_max_spots IS NOT NULL AND v_max_spots <> 0 THEN
    SELECT count(*) INTO v_current_active FROM bookings
...
BEGIN
  ...
  -- Bugfix (Sarah 2026-08-18): erst PRÜFEN ob wirklich ein Platz frei ist,
  -- bevor überhaupt versucht wird nachzurücken.
  SELECT COALESCE(s.max_spots, c.max_spots) INTO v_max_spots
    FROM sessions s LEFT JOIN courses c ON c.id = s.course_id
   WHERE s.id = p_session_id;
